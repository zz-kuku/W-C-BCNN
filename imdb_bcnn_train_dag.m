...
%% zz_uncle
if strcmp(opts.dataset,'dog') || strcmp(opts.dataset, 'endoscope')
    opts.inittrain.numEpochs = 50 ;
    
% % %     for i = min(imdb.images.label):max(imdb.images.label)
% % %         isAugmentation(i) = numel(find(imdb.images.label(ismember(imdb.images.set,1))==i)) <= 50;
% % %     end
% % %     opts.isAugmentation = isAugmentation;
% % % else
% % %     opts.isAugmentation = [];
    opts.train.isClickW = 1;
    opts.isClickconn = 1;
else
    opts.train.isClickW = 0;
    opts.isClickconn = 0;
end
...
if simplenn    
    ...
    for i=1:numel(train_bopts)
        ...
        train_bopts(i).isClickconn = opts.isClickconn;
% % %         train_bopts(i).isAugmentation = opts.isAugmentation;
        ...
        val_bopts(i).isClickconn = opts.isClickconn;
% % %         val_bopts(i).isAugmentation = opts.isAugmentation;
    end
