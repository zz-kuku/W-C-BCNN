...
%       state.getBatch = getBatch_val;
  ...
  if opts.isClickW
    updateWeigh(epoch, state.imdb, state.train, stats.train(epoch).loss);
    
    % save loss
    if ~exist(fullfile(opts.expDir,'loss'),'dir')
        mkdir(fullfile(opts.expDir,'loss'));
    end
    [~,loc] = ismember(state.train,opts.train);
    loss = zeros(numel(state.train),1);
    loss(loc) = stats.train(epoch).loss;
    save(fullfile(opts.expDir,'loss',['loss' num2str(state.epoch) '.mat']),'loss');
  end
...
subset = state.(mode) ;
% subset = subset(1:200);
loss = zeros(numel(subset),1);
start = tic ;
num = 0 ;

if state.epoch == 1 || ~opts.isClickW || ~strcmp(mode,'train')
    data_W = ones(numel(subset),1);
elseif opts.isClickW
    load(fullfile(state.imdb.clickDir,['data_W' num2str(state.epoch-1) '.mat']),'data_W');
end
...
%         weightLoss(ismember(subset(t:batchEnd),batch)) = data_W(batch);
    % 根据打乱后的batch选取对应顺序权重
    if strcmp(mode,'train')
        [~, loc] = ismember(batch,opts.train);
    else
        [~, loc] = ismember(batch,opts.val);
    end
    weightLoss = data_W(loc);
    save(['weightLoss' num2str(labindex) '.mat'],'weightLoss');
    
    if strcmp(mode, 'train')
% % %       net.vars(net.getVarIndex('l_1')).precious=1;
      net.accumulateParamDers = (s ~= 1) ;
      net.eval(inputs, opts.derOutputs) ;
% % %       code_b = squeeze(gather(net.vars(net.getVarIndex('l_1')).value));
% % %       for i = 1:numel(batch)
% % %         code = code_b(:,i);
% % %         savefast(fullfile(opts.expDir, 'bcnn_ft', ['bcnn_ft_', num2str(batch(i), '%06d')]), 'code');
% % %       end
% % %       net.vars(net.getVarIndex('l_1')).precious=0;
    else
      net.eval(inputs) ;
    end
  end
  ...
  % extract loss
  if strcmp(mode,'train') && opts.isClickW
      load(['loss_t' num2str(labindex) '.mat'],'loss_t');
      [~, idx] = ismember(batch,opts.train);
      loss(idx) = gather(loss_t);
  end
...
if strcmp(mode,'train')  && opts.isClickW
    [~, loc] = ismember(subset, opts.train);
    stats.loss = loss(loc);
end
