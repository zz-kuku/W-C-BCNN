...
opts.topK = 5 ;
if size(X,3)<opts.topK, opts.topK=size(X,3);end;
...
if strcmp(opts.loss ,'softmaxlog')
  load(['weightLoss' num2str(labindex) '.mat']);
  instanceWeights(1,1,1,:) = weightLoss(:);
end
% --------------------------------------------------------------------
% Do the work
% --------------------------------------------------------------------
...
if nargin <= 2 || isempty(dzdy)
  switch lower(opts.loss)
    ...
    case 'softmaxlog'
      ...
      [~, idx] = max(X,[],3);
      idx = reshape(idx,size(c));
      result_c = squeeze(idx-c);
      save(['result_c' num2str(labindex) '.mat'],'result_c');
      loss_t = t;
      save(['loss_t' num2str(labindex) '.mat'],'loss_t');
    case 'mhinge'
      t = max(0, 1 - X(ci)) ;
    case 'mshinge'
      Q = X ;
