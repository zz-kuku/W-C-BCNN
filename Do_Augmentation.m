...
mdir = endoscopeDir;
% dogDir = pwd;
Maxnum = 300; %%%训练每类最多样本数
Minnum = 0; %%%训练每类最少样本数，你自己调整下
% CropAug = 50;
...
%         ind_test = ind_test(1:floor(Maxnum*0.2));
        ...
        imageNames_ALL = [imageNames_ALL; imageNames_Tr(index([ind_train; ind_val; ind_test]))];
            ...
            %设定切、加色、旋转、加躁的翻倍数
            CropAug = 2;ColorAug = 2;RotAug = 2;NoiseAug= 0;BlockAug= 0;MixAug = 2;
...
% % %如果没有验证集，拿原来的训练作为验证，Augmentation后的图片作为训练
% % imageNames_ALL = [imageNames_ALL; imageNames(index_Tr)];
% % t = numel(index_ALL)+1;
% % index_ALL = [index_ALL; index_Tr];
