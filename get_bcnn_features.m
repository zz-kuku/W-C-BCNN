function code= get_bcnn_features(net, im, fea_click, varargin)
...
for k=1:numel(im)
    ...
    for s=1:numel(opts.scales)
        ...
        if isDag
            ...
            if ~isempty(fea_click)
                inputs{end+1} = 'click_vector';
                temp{1} = fea_click{1}(:,k);
                temp{2} = fea_click{2}(k);
                inputs{end+1} = temp;
            end
            if isTwoNet
                if strcmp(net.device, 'gpu')
...
%             feat = net.vars(net.getVarIndex('l_1')).value;
            feat = net.vars(net.getVarIndex('i_c')).value;
%             feat = net.vars(net.getVarIndex('d_1')).value;
        else
            if net.useGpu
                im_resizedA = gpuArray(im_resizedA);
            end
...
if isDag
    ...
    input = net.getVarIndex({'input', 'netb_input','click_vector'});
