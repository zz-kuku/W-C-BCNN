...
% zz_uncle
% 
%layerName = 'bilinear2maxP';
%input = bpoutput;
%output = 'b2m';
%net.addLayer(layerName,Bilinear2MaxPooling(),{input},output);

% MaxPooling
%layerName = 'maxPooling';
%input = output;
%output = 'm_P';
%net.addLayer(layerName,dagnn.Pooling('method','max','poolSize',[4,4],'pad',[0,1,0,1], ...
%    'stride',[4,4]),input,output);

%layerName = 'maxP2bilinear';
%input = output;
%bpoutput = 'm2b';
%net.addLayer(layerName,MaxPooling2Bilinear(),{input},bpoutput);
...
% ------------------------------------------------------------------------------------
% zz_uncle
layerName = 'click_conn';
input = cat(2,{bpoutput},{'click_vector'});
bpoutput = 'i_c';
net.addLayer(layerName,Clickconnection(),input,bpoutput);
...
input = bpoutput;
bpoutput = 'd_1';
net.addLayer(layerName,dagnn.DropOut('rate',0.1),{input},bpoutput);
...
% % %         if ~isempty(opts.isAugmentation)
% % %             train_batch = train;
% % %         end
        ...
        if ~exist(opts.nonftbcnnDir)
            ...
            for i=1:numel(bopts)
                ...
                bopts(i).isClickconn = opts.isClickconn;
% % %                 bopts(i).isAugmentation = opts.isAugmentation;
            end
            ...
            for t=1:batchSize:numel(train)
                ...
                net.mode = 'test' ;
                net.eval(input);
                fIdx = net.getVarIndex(bpoutput);
                code_b = net.vars(fIdx).value;
                code_b = squeeze(gather(code_b));
                
                for i=1:numel(batch)
                    code = code_b(:,i);
                    savefast(fullfile(opts.nonftbcnnDir, ['bcnn_nonft_', num2str(batch(i), '%06d')]), 'code');
                end
            end
